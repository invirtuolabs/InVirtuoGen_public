...
def write_latex_tables(results: dict, outpath: str = "comparison_tables.tex", include_std: bool = True):
    ...
    Std shows variation across seeds. Average uses error propagation.
    ...
    with open(outpath, "w") as f:
        ...
        # Table setup
        f.write("  \\caption{Average performance across five fragment‐constrained generation tasks: motif extension, linker design, superstructure generation, scaffold morphing, and scaffold decoration. In our setup, scaffold decoration is identical to linker design, so results are shared. For all tasks, InVirtuoGEN achieves the highest average on all tracked metrics.}\n")
        f.write("  \\small\n")
        ...
        for task, methods in results.items():
            task_display = (task.replace("linker", r"\makecell[l]{Linker Design/\\Scaffold Morphing}")
                                .replace("motif", "Motif Extension")
                                .replace("decoration", "Scaffold Decoration")
                                .replace("superstructure", "Superstructure Design"))
        ...
        # Average block with error propagation
        ...
        for method in method_names_sorted:
            ...
            for metric in metrics:
                vals = method_metric_values[method][metric]
                stds = method_metric_stds[method][metric]

                if vals:
                    # Average
                    num_terms = len(vals)
                    mean = sum(vals) / num_terms

                    # Error propagation for  average
                    variance = sum((std)**2 for std in stds)
                    propagated_std = np.sqrt(variance) / num_terms if variance > 0 else 0

                    avg_values[method][metric] = mean
                    avg_stds[method][metric] = propagated_std
                else:
                    avg_values[method][metric] = float("nan")
                    avg_stds[method][metric] = 0
...
def load_reference_metrics(reference_metrics_path="references/reference_metrics.csv"):
    """Load reference metrics from CSV."""
...
if __name__ == "__main__":
    ...
    for task in tasks:
        ...
        out["InVirtuoGen"] = merged_results[task]["InVirtuoGen"]
    ...
    with open("results/fragment_constrained/results_with_std.json", "w") as f:
        json.dump(merged_results, f, indent=4)

    # Generate LaTeX tables with std
    write_latex_tables(results_avg, outpath="results/fragment_constrained/comparison_tables.tex", include_std=True)

    # Also generate a version without std for cleaner appearance if needed
    write_latex_tables(results_avg, outpath="results/fragment_constrained/comparison_tables_no_std.tex", include_std=False)
